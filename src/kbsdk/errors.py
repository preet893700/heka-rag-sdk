"""Exception hierarchy. Everything the SDK raises on purpose derives from `KbsdkError`."""
...
class ConfigError(KbsdkError, ValueError):
    """The configuration is invalid or cannot be loaded."""


class AccessDeniedError(KbsdkError, PermissionError):
    """A question was asked without the identity the access policy requires."""


class BudgetExceededError(KbsdkError):
    """A configured cost or call budget was used up."""


class UnknownProviderError(KbsdkError, KeyError):
...
class MissingExtraError(KbsdkError, ImportError):
    ...
    def __init__(self, stage: str, name: str, extra: str, cause: ImportError) -> None:
        ...
        super().__init__(
            ...
            f"(missing: {cause.name or cause}). Install them with: pip install 'kbsdk[{extra}]'"
        )
