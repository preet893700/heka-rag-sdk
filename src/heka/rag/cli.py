"""Command line: heka-rag ingest | ask | inspect | eval run | eval check | eval gen | eval ablate | serve | presets."""
...
from heka.rag.config import RAGConfig, available_presets
from heka.rag.env import load_env_file
from heka.rag.errors import HekaRagError
from heka.rag.eval.ablation import run_ablation
from heka.rag.eval.dataset import load_dataset
from heka.rag.eval.judge import LLMJudge
from heka.rag.eval.report import EvalReport
from heka.rag.eval.runner import EvalRunner, run_retrieval_eval
from heka.rag.eval.synth import SynthOptions, generate_dataset
...
from heka.rag.usage import MeteredLLM, track_usage
...
def _cmd_eval_run(args: argparse.Namespace) -> int:
    ...
    if not dataset.cases:
        raise HekaRagError("No cases to run after applying --split/--tag/--limit.")

    stamp = datetime.now().strftime("%Y%m%d-%H%M%S")
    out_dir = Path(args.out) if args.out else kb.dir / "runs" / f"{stamp}-{slugify(dataset.name)}"
    if args.resume and not args.out:
        raise HekaRagError("--resume needs --out pointing at the earlier run's folder.")
...
def _cmd_eval_gen(args: argparse.Namespace) -> int:
    ...
    if out.exists() and not args.force:
        raise HekaRagError(f"{out} already exists; pass --force to overwrite it.")
    context = None
    if args.context:
        try:
    ...
    if not result.dataset.cases:
        raise HekaRagError("No questions passed the checks. " + " ".join(result.warnings))
...
def _build_auth(args: argparse.Namespace) -> Any:
    from heka.rag.server import ApiKeyAuth, JwtAuth, NoAuth
    ...
    if args.auth == "none":
        ...
        return NoAuth()
    if args.auth == "apikey":
        keys = _required_env("HEKA_RAG_API_KEYS", "--auth apikey needs the allowed keys").split(",")
        return ApiKeyAuth(keys, trust_caller_context=args.trust_caller_context)
    if args.trust_caller_context:
        raise HekaRagError("--trust-caller-context only applies to --auth apikey.")
    hmac_alg = args.jwt_algorithm.upper().startswith("HS")
    key = (
        _required_env(
            "HEKA_RAG_JWT_SECRET", "--auth jwt with an HS* algorithm needs the shared secret"
        )
        if hmac_alg
        else _required_env(
            "HEKA_RAG_JWT_PUBLIC_KEY", "--auth jwt needs the issuer's public key (PEM)"
        )
    )
    return JwtAuth(
        key,
...
def _cmd_serve(args: argparse.Namespace) -> int:
    from heka.rag import server

    if args.debug_responses and args.host not in LOOPBACK_HOSTS:
        raise HekaRagError(
            "--debug-responses exposes retrieved passages; only use it on localhost."
        )
    configs = [RAGConfig.from_file(path) for path in args.config]
    admin_key = os.environ.get("HEKA_RAG_ADMIN_KEY", "").strip() or None
...
def _load_variants(path: str) -> dict[str, dict[str, Any]]:
    try:
        ...
        raise HekaRagError(f"Cannot read variants file {path}: {exc.strerror or exc}") from exc
    if isinstance(data, dict) and isinstance(data.get("variants"), dict):
        data = data["variants"]
    if not isinstance(data, dict) or not data:
        raise HekaRagError("The variants file must map variant names to config overrides.")
    for name, overrides in data.items():
        if overrides is None:
            data[name] = {}
        elif not isinstance(overrides, dict):
            raise HekaRagError(f"Variant {name!r} must be a mapping of config overrides.")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="heka-rag", description="Configurable RAG SDK")
    ...
    serve = commands.add_parser(
        "serve", help="serve the agents over HTTP (needs heka-rag-sdk[server])", parents=[env]
    )
    serve.add_argument(
        "-c",
        "--config",
    ...
    serve.add_argument(
        ...
        help="how callers prove who they are (secrets come from HEKA_RAG_API_KEYS / HEKA_RAG_JWT_SECRET / "
        "HEKA_RAG_JWT_PUBLIC_KEY; HEKA_RAG_ADMIN_KEY enables POST .../ingest)",
    )
    serve.add_argument(
