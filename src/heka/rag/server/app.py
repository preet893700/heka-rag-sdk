"""The REST server: a thin, authenticated HTTP layer over `Agent`. Needs `pip install 'heka-rag-sdk[server]'`.
...
* Identity comes from the credential (see `heka.rag.server.auth`), never from the request body unless the
...
from heka.rag.errors import AccessDeniedError, BudgetExceededError, ConfigError, HekaRagError
from heka.rag.knowledge_base import KnowledgeBase
from heka.rag.server.auth import Authenticator, AuthError, NoAuth, secrets_match
from heka.rag.text import slugify
from heka.rag.types import Citation, Message, RequestContext

log = logging.getLogger("heka.rag.server")
