...
class OtelTracer:
    ...
    def emit(self, event: TraceEvent) -> None:
        ...
        span = self._tracer.start_span(f"heka.rag.{clean.stage}.{clean.name}", start_time=start_ns)
        span.set_attribute("heka.rag.stage", clean.stage)
        for key, value in clean.data.items():
            if value is not None:
                span.set_attribute(f"heka.rag.{key}", _attribute(value))
